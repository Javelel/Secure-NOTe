...
public class EncryptionService {
	...
	public String encrypt(String content, String password) {
		try {
			...
			final byte[] encryptedBytesWithSaltAndIV = new byte[encryptedBytes.length + SALT_LENGTH + IV_LENGTH];
			System.arraycopy(salt, 0, encryptedBytesWithSaltAndIV, 0, SALT_LENGTH);
			System.arraycopy(iv, 0, encryptedBytesWithSaltAndIV, SALT_LENGTH, IV_LENGTH);
			System.arraycopy(encryptedBytes, 0, encryptedBytesWithSaltAndIV, SALT_LENGTH + IV_LENGTH, encryptedBytes.length);

			return Base64.getEncoder().encodeToString(encryptedBytesWithSaltAndIV);
	...
	private Cipher initializeCipher(String password, byte[] salt, byte[] iv, int mode) {
		try {
			...
			final PBEKeySpec keySpec = new PBEKeySpec(password.toCharArray(), salt, KEY_ITERATIONS, KEY_LENGTH);
